...
async def on_emoji_input(
    ...
    if message.text is None or not message.text.strip().isdigit():
        await notifier.notify_user(user, i18n_key="ntf-menu-editor.invalid-emoji")
        return
    ...
    button = dialog_manager.dialog_data["button"]
    button = retort.load(button, MenuButtonDto)
    button.emoji_id = emoji_id
    dialog_manager.dialog_data["button"] = retort.dump(button)

    logger.info(f"{user.log} Updated menu button '{button.index}' emoji_id to '{emoji_id}'")

    await dialog_manager.switch_to(RemnashopMenuEditor.BUTTON)


@inject
async def on_emoji_clear(
    callback: CallbackQuery,
    widget: Button,
    dialog_manager: DialogManager,
    retort: FromDishka[Retort],
    ...
    button.emoji_id = None
    dialog_manager.dialog_data["button"] = retort.dump(button)

    logger.info(f"{user.log} Cleared emoji_id for menu button '{button.index}'")
